%! Author = CHR
%! Date = 08.05.2023
\section{Data}
\label{exp:data}

\begin{table}[]
	\caption{Sample RBF scale from beta distribution}
	\begin{tabular}{c c c}
		\toprule
		& Version A & Version B\\
		\midrule
		Alpha & 2 & 2\\
		Beta & 10 & 5  \\\midrule
		Scale Mean & 0.15 & 0.3\\\bottomrule
	\end{tabular}
\end{table}
\begin{table}[]
	\caption{Number of grid points}
	\begin{tabular}{c c c c c c}
		\toprule
		& Train & \multicolumn{4}{c}{Test}\\
		\midrule
		Sample Size & 100K & \multicolumn{4}{c}{1K}\\
		Grid Points & 5-50 & 5 & 10 & 50 & 100  \\\bottomrule
	\end{tabular}
\end{table}

\begin{figure}
	\centering
	\input{abbildungen/beta}
\end{figure}

\begin{itemize}
	\item Number of points in grid
	\begin{itemize}
		\item Train: $n\sim \mathcal{U}(5,50)$
		\item Test: Given exact number of points
	\end{itemize}
	\item $S:=\{x_1, \ldots, x_{128}\}$\\ Draw $\{x_1, \ldots, x_n\} \sim \text{UniformSubset}(S, n)$
\end{itemize}

	\begin{itemize}
	\item Draw $\sigma \sim \mathcal{N}(0,0.1)$
	\item Draw noise for each datapoint from $\mathcal{N}(0,\sigma)$
\end{itemize}

\section{Experiments}
\label{exp:experiments}

	\begin{figure}
	\centering
	\resizebox{!}{0.40\textheight}{
		\input{abbildungen/plots/GP/63}
		\input{abbildungen/plots/GP/57}
	}
	\resizebox{!}{0.40\textheight}{
		\input{abbildungen/plots/GP/62}
		\input{abbildungen/plots/GP/63_100}
	}
\end{figure}

	\begin{figure}
	\centering
	\resizebox{0.95\textwidth}{!}{
		\input{abbildungen/plots/training/final_loss}
		\input{abbildungen/plots/training/final_val_loss}
	}
\end{figure}

	\begin{figure}
	\centering
	\resizebox{0.95\textwidth}{!}{
		\input{abbildungen/plots/training/lr}
		\input{abbildungen/plots/training/final_dim}
	}
\end{figure}

	\begin{table}[]
	\caption{RMSE Loss for mean compared to target values}
	\begin{tabular}{c c c c c}
		\toprule
		Dataset & Grid Points & GP & model A & model B\\
		\midrule
		\multirow{4}{*}{A (2,10)} & 5 & 0.91 & 0.72 & 0.71\\
		&10& 0.78 & 0.50 & 0.49\\ 
		&50 & 0.51 & 0.26 & 0.29\\
		&100 & 0.32 & 0.24 & 0.28\\\midrule
		\multirow{4}{*}{B (2,5)} & 5 & 0.84 & 0.55 & 0.55\\
		& 10 & 0.63 & 0.37 & 0.36\\
		& 50 & 0.28 & 0.14 & 0.16\\
		& 100 & 0.16 & 0.13 & 0.16\\\bottomrule
	\end{tabular}
\end{table}

%	\begin{figure}
%	\centering
%	\resizebox{0.95\textwidth}{!}{
%		\input{figures/plots/comparison/51}
%		\input{figures/plots/comparison/63}
%	}
%\end{figure}
%
%	\begin{figure}
%	\centering
%	\resizebox{!}{0.40\textheight}{
%		\input{figures/plots/modelA/61}
%		\input{figures/plots/modelA/54}
%	}
%	\resizebox{!}{0.40\textheight}{
%		\input{figures/plots/modelA/25}
%		\input{figures/plots/modelA/42}
%	}
%\end{figure}

\section{Conclusion}
\label{exp:conclusion}
